    ...
    <section id="portfolio" className="py-24 bg-gray-50">
      <div className="container-custom">
        <div className="text-center mb-16">
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-gray-600 mt-4 max-w-xl mx-auto">
            {p.description}
          </motion.p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {p.projects.map((project: { name: string; niche: string; desc: string }, i: number) => (
            <motion.div
              key={projectPaths[i]}
            ...
            >
              <Link href={projectPaths[i]} className="group block">
                <div className="relative overflow-hidden rounded-2xl bg-white border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-medium text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full">{project.niche}</span>
                    ...
                    <p className="text-sm text-gray-500 mt-1 leading-relaxed">{project.desc}</p>
                  </div>
                </div>
              </Link>
            </motion.div>
